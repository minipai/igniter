...
describe("worker command boundary", () => {
  test("bare Todo creates its stable named worker and titled tab, confirms delivery, and returns effective profile and result", async () => {
    const h = await setup();
    const out = await workerCommand(["start", "sta-244"], h.ctx);
  ...
  test("concurrent and later start retries reuse one worker and one confirmed work order even after HEAD moves", async () => {
    const h = await setup();
    const results = await Promise.all([workerCommand(["start", "STA-244"], h.ctx), workerCommand(["start", "STA-244"], h.ctx)]);
    expect(results.every((r) => r.ok)).toBe(true);
    h.git.head = "defaced123";
    h.issue.stateId = "st-build";
    h.issue.labelIds = ["label-in-progress"];
    expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(true);
  ...
  test("start failure leaves Linear untouched and retries reuse the prepared tab", async () => {
    ...
    expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(false);
    expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(true);
  ...
  test("undelivered prompt fails without begin; retry sends the identical work order to the same worker", async () => {
    ...
    const failed = await workerCommand(["start", "STA-244"], h.ctx);
  ...
  test("restart performs a real rebuild with the effective model, preserves dirty work, and dedupes retries", async () => {
    ...
    const args = ["restart", "STA-244", "--harness", "codex", "--model", "gpt-5.6-sol", "--effort", "high"];
    const out = await workerCommand(args, h.ctx);
    expect(out.ok).toBe(true);
    ...
    expect((await workerCommand(args, h.ctx)).ok).toBe(true);
  ...
  test("restart validates an incompatible model before touching a live worker", async () => {
    const h = await setup();
    await workerCommand(["start", "STA-244"], h.ctx);
    expect((await workerCommand(["restart", "STA-244", "--harness", "codex", "--model", "provider/model"], h.ctx)).ok).toBe(false);
  ...
  test("restart stop failure retries the saved effective profile and actual rebuild", async () => {
    ...
    const args = ["restart", "STA-244", "--harness", "codex", "--model", "gpt-5.6-sol"];
    expect((await workerCommand(args, h.ctx)).ok).toBe(false);
    expect((await workerCommand(args, h.ctx)).ok).toBe(true);
    expect(h.workspaces.agents).toHaveLength(1);
  ...
  test("failed cross-harness stop keeps permission answers bound to the live old harness", async () => {
    const h = await setup();
    await workerCommand(["start", "STA-244"], h.ctx);
    await workerCommand(["restart", "STA-244", "--harness", "claude", "--model", "sonnet"], h.ctx);
    const oldPane = h.workspaces.agents[0]!.paneId;
    h.workspaces.failNext("agent.stop");
    const restart = ["restart", "STA-244", "--harness", "codex", "--model", "gpt-5.6-sol"];
    expect((await workerCommand(restart, h.ctx)).ok).toBe(false);
    expect(h.workspaces.agents[0]!.paneId).toBe(oldPane);
    const reused = await workerCommand(["start", "STA-244"], h.ctx);
    expect(reused.ok).toBe(true);
    expect(reused.data).toMatchObject({ model: "sonnet" });
    expect(h.workspaces.agents[0]!.paneId).toBe(oldPane);
    expect((await workerCommand(["answer", "STA-244", "y"], h.ctx)).ok).toBe(true);
  ...
  test("Done cleanup is explicit, validates receipt, and retains dirty work after stopping workers", async () => {
    ...
    h.issue.labelIds = [];
    expect((await workerCommand(["stop", "STA-244"], h.ctx)).ok).toBe(false);
    ...
    h.git.statusPorcelain = " M unfinished.ts";
    const out = await workerCommand(["stop", "STA-244"], h.ctx);
  ...
  test("Done cleanup preserves extra user panes and their checkout", async () => {
    ...
    const result = await workerCommand(["stop", "STA-244"], h.ctx);
  ...
  test.each(["missing-progress", "conflicting-progress", "canceled"])("existing workers remain controllable when Linear is %s", async (condition) => {
    ...
    expect((await workerCommand(["send", "STA-244", "--role", "build", "keep existing work"], h.ctx)).ok).toBe(true);
    expect((await workerCommand(["answer", "STA-244", "--role", "build", "n"], h.ctx)).ok).toBe(true);
    expect((await workerCommand(["stop", "STA-244", "--role", "build"], h.ctx)).ok).toBe(true);
    expect(h.workspaces.agents).toHaveLength(0);
    expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(false);
    readsOnly(h.client);
  });
  ...
  test("same-ticket multiple roles require explicit targeting for send, answer, restart and stop", async () => {
    ...
    expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(true);
    for (const args of [["send", "STA-244", "hello"], ["answer", "STA-244", "y"], ["restart", "STA-244"], ["stop", "STA-244"]]) {
      const out = await workerCommand(args, h.ctx);
      expect(out.ok).toBe(false);
      expect(out.text).toContain("multiple workers");
    }
    expect((await workerCommand(["send", "STA-244", "--role", "build", "keep work"], h.ctx)).ok).toBe(true);
    expect(h.workspaces.promptsFor("builder-sta-244").at(-1)).toBe("keep work");
    expect((await workerCommand(["answer", "STA-244", "--role", "review", "n"], h.ctx)).ok).toBe(true);
    expect((await workerCommand(["stop", "STA-244", "--role", "build"], h.ctx)).ok).toBe(true);
  ...
  test("answer uses frozen effective harness after model switch and refuses a changing pane", async () => {
    ...
    expect((await workerCommand(["answer", "STA-244", "y"], h.ctx)).ok).toBe(false);
  ...
  test("blocked and complete stages cannot start or restart workers", async () => {
    for (const progress of ["label-blocked", "label-complete"]) {
      ...
      expect((await workerCommand(["start", "STA-244"], h.ctx)).ok).toBe(false);
      expect((await workerCommand(["restart", "STA-244"], h.ctx)).ok).toBe(false);
